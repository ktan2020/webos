...
/*globals _ $A PalmSystem Element */

/*$
 * Describes the Mojo.Controller.SceneController class
 * @class
 */
Mojo.Controller.SceneController = function(stageController, sceneArguments, remainingArguments) {
	...
	this.updateSceneScrollerSizeHandler = this.updateSceneScrollerSize.bind(this);
	...
	Mojo.require(constructorFunction, "The scene assistant '" + assistantName +
		"' is not defined. Did you remember to include it in sources.json?");
	
	var assistant;
	if (constructorFunction) {
		assistant = Mojo.createWithArgs(constructorFunction, remainingArguments);
		assistant.controller = this;
		this.assistant = assistant;
	}
	
	var controller = this;
	var assistantFunctionsToWrap = ["setup", "cleanup", "activate", "deactivate", "orientationChanged", "ready", "aboutToActivate"];
	if(assistant) {
		_.each(assistantFunctionsToWrap, function(functionName) {
			var delegateFunctionName = Mojo.Controller.assistantFunctionName(functionName);
			controller[delegateFunctionName] = controller.wrapAssistantFunction(assistant, functionName, sceneName);
		});
	} else {
		assistantFunctionsToWrap.each(function(functionName) {
			var delegateFunctionName = Mojo.Controller.assistantFunctionName(functionName);
	...
	this.unfocusOnTapHandler = this.unfocusOnTap.bind(this);
};



Mojo.Controller.SceneController.prototype = {
	...
	ASSISTANT_FAILURE: 'fail',

	/*
	 * Gets called post-construction by the stage controller once the scene
	 * element has been rendered.
	 */
	setSceneElement: function(sceneElement) {
		this.sceneElement = sceneElement;
		this.pushContainer(this.sceneElement, this.sceneContainerLayer);		
	},

	wrapAssistantFunction: function(assistant, functionName, sceneName) {
		var func = assistant[functionName];
		var stageName, stageType, args;
		
		if(func) {
			return function() {
				try {
					func.apply(assistant, arguments);
					return this.ASSISTANT_SUCCESS;
				} catch(e) {
					stageName = this.window.name;
					stageType = this.window._mojoStageType;

					try {
						args = "arguments were : " + JSON.stringify(Array.prototype.slice.call(arguments));
					} catch(e2) {
						args = "";
					}
					
					Mojo.Log.logException(e, "Exception caught in "+stageType+" stage : '"+stageName+"' in scene "+ sceneName+"'s "+ functionName +" method. "+args+".");

					if(stageType == Mojo.Controller.StageType.dashboard || 
						stageType === Mojo.Controller.StageType.popupAlert) {
							Mojo.Log.error('Closing stage.');
							Mojo.Controller.appController.closeStage(stageName);

					} 
					return this.ASSISTANT_FAILURE;
				}
			};
		} else {
			return Mojo.doNothing;
		}
	},
	...
	setup: function() {
		var stageType = Mojo.Controller.StageType;
		var scType = this.window._mojoStageType;
		var assistantResult;
		...
		assistantResult = this.assistantSetup();

		if(assistantResult === this.ASSISTANT_FAILURE) {
		...
		if(scType === stageType.card || scType === stageType.stackedCard ||
			scType === 'menu' || scType === stageType.dockMode) {
			this._installMenus();
		}
		...
		assistantResult = this.assistantReady();

		if(assistantResult === this.ASSISTANT_FAILURE) {
			return assistantResult;
		}

		_.defer(this.focusFirstElement.bind(this));
	},

	cleanup: function() {
		var i, activeServiceReqs, activeSubServiceReqs;
		...
		activeServiceReqs = this.activeServiceRequests;
		for(i= activeServiceReqs.length -1; i >= 0; i--) {
			activeServiceReqs[i].originalCancel();
		}

		this.activeServiceRequests = undefined;
	
		activeSubServiceReqs = this.activeSubscribedServiceRequests;
		for(i= activeSubServiceReqs.length -1; i >= 0; i--) {
			activeSubServiceReqs[i].originalCancel();
		}

		this.activeSubscribedServiceRequests = undefined;
	
		if (this.scrollingEnabled) {
		    Mojo.Dom.remove(this.sceneScroller);
			Mojo.removeAllEventListenersRecursive(this.sceneScroller);
		} else {
		    Mojo.Dom.remove(this.sceneElement);
	...
	aboutToActivate: function(synchronizer) {
		...
		if(this.assistant && this.assistant.aboutToActivate) {
			this.assistantAboutToActivate(synchronizer.wrap(Mojo.doNothing));
		}
	...
	activate: function(returnValue) {
		...
		Mojo.Event.listen(this.sceneElement, Mojo.Event.tap, this.unfocusOnTapHandler); //make sure that selection vs focus works properly!
		Mojo.Event.listen(this.sceneElement, Mojo.Event.listTap, this.unfocusOnTapHandler); //make sure that selection vs focus works properly!
	},
	
	/*$ @private */
	deactivate: function() {
		Mojo.Event.stopListening(this.window, 'resize', this.updateSceneScrollerSizeHandler);
		Mojo.Event.stopListening(this.sceneElement, Mojo.Event.tap, this.unfocusOnTapHandler);
		Mojo.Event.stopListening(this.sceneElement, Mojo.Event.listTap, this.unfocusOnTapHandler);
		this._active = false;
	...
	select: function(cssSelector) {
		return _.toArray(this.sceneElement.querySelectorAll(cssSelector));
	},
	...
	*/
	...
	/*$
	 * Checks if the given commander is in the stack.
	 * Used to prevent double adding a commander.
	 * @param {Object} cmdr commander to add.
	 */
	doesCommanderExist: function(cmdr) {
		return this._commanderStack.doesCommanderExist(cmdr);
	},
	...
	*/
	removeWatcher: function(watcher, model) {
		...
		if(model !== undefined) {
			...
			this._modelWatchers = _.select(this._modelWatchers, function(w){return w.who !== watcher;});
		}
	},
	...
	*/
	...
	toggleAppMenu: function() {
		if(this._menu) {
			this._menu.assistant.toggleAppMenu();
		}
	},
	...
	serviceRequest: function(url, optionsIn, resubscribe) {
		...
		_.extend(serviceRequestOptions, options);
	...
	*/
	remove: function(element) {
		var extendedElement = this.get(element);
		Mojo.Dom.remove(extendedElement);
	},
	...
	*/
	newContent: function(elementOrElements) {
		if (_.isArray(elementOrElements)) {
			var i, element;
			
			for(i = elementOrElements.length - 1; i>=0; i--) {
				element = elementOrElements[i];
