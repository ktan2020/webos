...
var HttpClientContainer = Class.create({
        ...
        _responseBufferDone_: function(f) {
            this.responseBuffer.position = 0;
            
            function writeReady() {
                if (this.responseWrote === this.responseBuffer.length) {
                    console.log("Write buffer to Socket complete!!! responseWrote: " + this.responseWrote + ", responseBuffer: " + this.responseBuffer.length);
                ...
                var writeCount = this.chan.writeUsingBuffer(this.responseBuffer, HTTPD['bufsize']);
                console.log(" ::: buffer write to channel: (" + writeCount + ") :::");
                this.responseBuffer.position += writeCount;
                this.responseWrote += writeCount;
            }
            
            if (this.chan) {
                this.chan.onwrite = writeReady.bind(this);
            } else {
                _Httpd_Log_("error", "XXX _responseBufferDone_: client channel == null XXX");
                throw "_responseBufferDone_: client channel == null";
            }
        },
        
        _writeBufferDone_: function(f) {
            this.writeBuffer.position = 0;
            
            function writeReady() {
                if (this.bytesWrote === this.bytesRead) {
                    console.log("Write buffer to Socket complete!!! bytesWrote: " + this.bytesWrote + ", writeBuffer.length: " + this.bytesRead);
                    this.chan.onwrite = undefined;
            ...
            if (this.chan) {
                ...
                _Httpd_Log_("error", "XXX _responseBufferDone_: client channel == null XXX");
                throw "_writeBufferDone_: client channel == null";
            }
        },
        ...
        _fileOnreadDone: function() {
            try {
                ...
                this._endBlock_();
                this._responseBufferDone_(function(){this._writeBufferDone_(function(){this._endBlock_(); this._done_();}.bind(this))}.bind(this));    
            } catch (err) {
