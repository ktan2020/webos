...
var GenericServer = Class.create({
        ...
        backEndDispatch: function(socket, channel) {
            ...
            if (socket && channel) {
                ...
                if (this.chan_opt == 'webOS.IOChannel.FLAG_NONBLOCK') {
                    if (this.handler) {
                        channel.onread = this.handler.curry(channel).bind(this);
                    } else {
         ...
         * 
         * Note:
         * Handler function must be function with 1 parameter. 
         *  E.g. To echo contents of client's sent message back:
         *  function(channel) { var read = channel.read(1024); channel.write(read); channel.flush(); }
         */
